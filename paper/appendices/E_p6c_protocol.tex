...
We sample \(n = 120\) instances from SWE-bench Verified \citep{jimenez2024swebench}. The sample is the deterministic prefix of the dataset under its native ordering after a single \texttt{random.Random(20251220).shuffle(instances)} call. The instance ids are emitted to \texttt{experiments/results/p6c/instance\_set.json}.

\subsection{E.2 Synthetic research-trail generator}\label{e.2-synthetic-research-trail-generator}

The generator (\texttt{experiments/v2/p6c/research\_evidence.py}) emits, for each instance, a list of \texttt{ResearchSnippet} objects under the contract:
...
\item
  Each stale snippet has \texttt{precision} \(\in [0.05, 0.30]\), points at a \emph{sibling} file path (a deterministic perturbation of the true target path), and is marked \texttt{stale=True}.
\item
  Each fresh snippet has \texttt{precision} \(\in [0.70, 0.95]\), points at the \emph{correct} target path, and is marked \texttt{stale=False}.
\item
  One fresh snippet's \texttt{superseded\_by\_id} points at one stale snippet, simulating the ``Stack-Overflow-superseded-by-blog'' pattern.
\item
  Snippets are then shuffled with \texttt{random.Random(seed\ +\ hash(instance\_id)\ \%\ 10\_000)} so the \emph{position} of the correct evidence in the research trail varies across seeds.
\end{itemize}

The generator is fully deterministic given \texttt{(instance\_id,\ seed)}; its tests live in \texttt{tests/test\_v2/test\_p6c\_research\_evidence.py}.

\subsection{E.3 The two arms}\label{e.3-the-two-arms}

Both arms operate under the same hard token budget \(B = 8192\) on the size of the \emph{research block} that the patch-generator sees. Both arms use the same \texttt{PatchSimulator} (see E.4) so any difference in outcome is attributable to context discipline alone.
...
Crucially, the harness's \texttt{compact} step prefers high-precision items first, so the in-budget block becomes a curated subset rather than a tail-truncated concatenation.
...
The simulator deterministically picks the \emph{first} \texttt{path.py} token mentioned in the (system + prompt) text. By construction, this means the output depends \emph{only} on the contents and order of the research block --- exactly the variable we want to isolate.
...
\tightlist
\item
  \textbf{Heuristic.} \texttt{target\_path\_hit\ =\ 1} iff \texttt{metadata{[}"anchored\_path"{]}\ ==\ ground\_truth\_path}. This is what we report in Section 10 because it is fully reproducible without Docker.
\item
  \textbf{Real SWE-bench (optional).} When \texttt{-\/-use-docker-eval} is passed, the patch is fed to the upstream SWE-bench evaluation harness which runs the repository's tests in a pinned Docker image. Pass/fail joins the per-instance row.
\end{itemize}
...
\texttt{experiments/results/p6c/summary.json} contains:
...
with \texttt{per\_instance} and \texttt{per\_model\_seed} arrays for downstream P7 figures (notably F10 and F11).
