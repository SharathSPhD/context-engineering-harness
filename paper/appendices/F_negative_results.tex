\section{Negative and Null Results}\label{negative-and-null-results}
...
\subsection{\texorpdfstring{Negative --- \texttt{PrecisionWeightedRAG} (v0)}{Negative --- PrecisionWeightedRAG (v0)}}\label{negative-precisionweightedrag-v0}
...
\textbf{Why it failed.} Three reasons surfaced empirically. \textbf{No calibration}: it reported binary ``agree/disagree'' without a calibrated posterior, and ECE ran at \(\sim 0.18\) on H2. \textbf{No conflict detection}: conflicts that should have triggered sublation merely returned the higher-precision answer silently, leaving the agent with no signal to \emph{audit} the disagreement. \textbf{No witness}: decisions were not traceable, so the agent could not later explain \emph{why} one source had been preferred.
...
\subsection{\texorpdfstring{Negative --- single-stage \texttt{BuddhiAgent}}{Negative --- single-stage BuddhiAgent}}\label{negative-single-stage-buddhiagent}
...
\subsection{Negative --- naive LRU compaction}\label{negative-naive-lru-compaction}
...
\subsection{Negative --- single-source-of-truth Khyātivāda annotator}\label{negative-single-source-of-truth-khyux101tivux101da-annotator}
...
\subsection{Null --- vLLM Qwen3-1.7B vs.~heuristic surprise backend}\label{null-vllm-qwen3-1.7b-vs.-heuristic-surprise-backend}
...
\subsection{Negative --- early plugin tried to inline ContextStore into prompt}\label{negative-early-plugin-tried-to-inline-contextstore-into-prompt}
