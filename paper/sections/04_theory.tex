...
\texttt{condition} is a free-text string like \texttt{"as\ of\ 2024-09-Django-5.0"} or \texttt{"on\ POSIX,\ Python\ ≥\ 3.11"}. The MCP tool \texttt{insert(qualificand,\ qualifier,\ condition,\ precision,\ source)} is the only way evidence enters the store. Two items that share \texttt{qualificand} but differ in \texttt{condition} \emph{do not conflict}; two items that share \texttt{qualificand} \emph{and} \texttt{condition} but disagree on \texttt{qualifier} \emph{do}. This is the operational definition of a contradiction the harness can act on (Section 4.3).
...
\tightlist
\item
  \textbf{\texttt{ManasAgent}} receives the user query and the \texttt{ContextStore} snapshot, and returns \emph{which items it has elected to attend to and under which conditions}. Its output is structured JSON: \texttt{\{attended:\ {[}item\_id,\ ...{]},\ conditions:\ {[}str,\ ...{]},\ filter\_reasons:\ {[}str,\ ...{]}\}}. Manas is \emph{forbidden} from emitting a final answer.
\item
...
\textbf{LLM operationalization.} The harness instantiates a \texttt{SakshiKeeperAgent} (Section 6.3) and a \texttt{SakshiPrefix} skill (Section 6.2). On every turn, the keeper appends to a persistent JSON-lines \texttt{witness\_log.jsonl} an immutable record of:
...
\textbf{LLM operationalization.} A few-shot Claude-side \texttt{KhyativadaClassifier} (Section 5.5) tags every Buddhi output with one of seven labels (six classes plus \texttt{none}). The classifier is grounded by a 3,000-example, two-rater annotated corpus (Section 11.2 / Appendix E) at Cohen's κ = 0.736 (``substantial'').
...
\bottomrule\noalign{}
\endlastfoot
Pratyakṣa & Visible-context-only grounding discipline & All \texttt{retrieve\_*} tools & All hypotheses (architectural commitment) \\
Avacchedaka & Typed limitor \texttt{(qualificand,\ qualifier,\ condition,\ precision)} & \texttt{insert}, \texttt{retrieve\_by\_qualifier} & H2, H5 \\
Bādha & Supersede-with-provenance under shared limitor & \texttt{sublate\_with\_evidence} & H4, H5, P6-B, P6-C \\
Manas & Pre-judgment attention-selection sub-agent & \texttt{ManasAgent} skill & H3 \\
Buddhi & Determinative judgment + khyāti tagging sub-agent & \texttt{BuddhiAgent} skill & H3, H6 \\
Sākṣī & Write-once cross-turn witness log & \texttt{SakshiKeeperAgent}, \texttt{SakshiPrefix} & All (audit invariant) \\
Khyātivāda & 6-class hallucination classifier & \texttt{classify\_khyati} & H6 \\
