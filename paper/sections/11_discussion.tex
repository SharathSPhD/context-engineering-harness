...
The Cohen's κ = 0.736 reported in §8.6 is between \emph{two automated annotators}: a deterministic heuristic and a simulated LLM-as-judge built from the few-shot Claude classifier whose prompts are documented in \textbf{Appendix D} (IAA numbers are in §8.6, not Appendix E). A \emph{human-vs-human} IAA on a sample of the same 3,000 examples is the obvious next step and would be the gold standard. Our preliminary read --- by the author, on a 200-example random sample --- agrees with the consensus label in 81\% of cases (κ ≈ 0.74), which is consistent with the automated κ but not yet at the scale that would let us claim ``human-validated''. This is the cleanest near-term improvement to make.
