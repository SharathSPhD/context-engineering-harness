\section{5 · The Pratyakṣa Harness: System Architecture}\label{the-pratyakux1e63a-harness-system-architecture}
...
The \texttt{ContextStore} is a typed, append-mostly key-value store that lives outside the LLM and is exposed to it via MCP. Its row schema is exactly the \emph{avacchedaka} tuple of Section 4.2:
...
Insertion is via a single MCP tool, \texttt{context\_insert(...)}. Retrieval is via \texttt{context\_retrieve(...)} (with optional \texttt{qualificand}, \texttt{qualifier\_substring}, and \texttt{condition} filters) and \texttt{context\_get(...)} for direct id fetch; none of these return \texttt{bādhita} or \texttt{evicted} rows by default. A debug flag exposes them for audit.
...
\item
  \textbf{Conditions are first-class.} Two rows that share \texttt{qualificand} and disagree on \texttt{qualifier} are \emph{not} yet in conflict if their \texttt{condition} fields differ. This is the avacchedaka commitment.
\item
  \textbf{Provenance never disappears.} Sublation rewrites \texttt{status}, never deletes. Eviction (under hard budget pressure) writes to a sidecar \texttt{evicted\_log.jsonl} so the witness can replay the eviction reason.
\end{enumerate}

\subsection{5.2 The AvacchedakaQuery module}\label{the-avacchedakaquery-module}

\texttt{context\_retrieve} with \texttt{qualificand} and \texttt{condition} set walks the \texttt{ContextStore} and returns the \emph{set} of items matching \texttt{qualificand} whose \texttt{condition} is \emph{consistent} with the supplied condition. Consistency is decided by a small rule engine:
...
\item
  \textbf{Temporal subsumption} wins when one condition is a strictly tighter time-window of the other (e.g.~\texttt{"Django\ ≥\ 5.0"} subsumes \texttt{"Django\ 5.0.4"}).
\item
  \textbf{Lexical-prefix subsumption} for versioned platforms (\texttt{"POSIX"} subsumes \texttt{"POSIX,\ Python\ ≥\ 3.11"}).
\item
  \textbf{Otherwise}, items are returned in \emph{both} groups and the conflict is left to be resolved downstream by Buddhi, possibly via \texttt{sublate\_with\_evidence}.
\end{itemize}
...
Conflict resolution proceeds in two steps. First, \texttt{sublate\_with\_evidence(target\_id,\ by\_id,\ reason)} performs the Vedānta operation of Section 4.3. Second, when the agent must commit to a single posterior probability for a contested claim --- for example, ``is the user's setting \texttt{enable\_x\ =\ True}?'' --- the harness offers a \textbf{Bayesian Beta-Bernoulli aggregator} in place of the original \emph{PrecisionWeightedRAG} of v0:
...
\tightlist
\item
  The host agent enters the harness's \texttt{manas\_step(query)} which returns the Manas JSON.
\item
  The host agent enters \texttt{buddhi\_step(query,\ manas\_output)} which returns the final answer + khyāti class.
\item
  The host agent enters \texttt{sakshi\_record(...)} which appends the immutable witness log entry.
\end{enumerate}
...
Sample exemplars are curated for the few-shot \emph{experiment} path to maximize within-class lexical diversity (e.g., asatkhyāti includes both ``fabricated CVE'' and ``fabricated stack-trace line'', not just one). In both stacks, heuristic guardrails override the model-proposed label when (a) the answer cites a \texttt{qualificand} not present in any \texttt{ContextStore} item (force \texttt{asatkhyāti}), or (b) the answer's \texttt{qualifier} is the negation of an item the answer cites by id (force \texttt{viparītakhyāti}). These guardrails are \emph{additive} --- they only flip \texttt{none} to a class, never flip one class to another.
...
\tightlist
\item
  Summarises the just-closed episode into a single new \texttt{ContextItem} with \texttt{qualifier="episode\_summary"} and \texttt{precision\ =\ mean(precisions\ of\ contained\ items)}.
\item
  Marks all contained items as \texttt{status="evicted"} (provenance retained).
\item
...
Rules are as in Section 4.7. Implemented as a periodic background MCP tool \texttt{compact(strategy:\ Literal{[}"adaptive",\ "lru",\ "none"{]})} that the agent or a lifecycle hook can invoke. The default policy is \texttt{adaptive}; the H7 study (Section 8.7) compares all three.

\subsection{5.8 TokenBudgetWatchdog}\label{tokenbudgetwatchdog}

A small but indispensable module. The watchdog tracks input/output tokens by category (system, retrieval, tool, completion) against a configured budget (default 60\% of model context window) and exposes one MCP tool, \texttt{budget\_status()}, returning:
...
It is also wired into a \texttt{pretooluse-budget.sh} hook (Section 6.4) that \textbf{logs} budget pressure by default; optional strict refusal is available when \texttt{PRATYAKSHA\_BUDGET\_STRICT=1}.
...
\item
  \textbf{Manas} is called: it inspects the \texttt{ContextStore} snapshot via \texttt{context\_retrieve} / \texttt{context\_window} and returns its attended subset.
\item
  \textbf{Buddhi} is called with that subset. It may issue \textbf{\texttt{sublate\_with\_evidence}} for any conflicts it spots before answering.
\item
  \textbf{\texttt{classify\_khyativada}} is run on Buddhi's draft answer; if \texttt{khyati\_class\ !=\ "none"} and \texttt{confidence\ \textgreater{}\ τ\_self\_check} (default 0.7), Buddhi is asked to revise once.
\item
