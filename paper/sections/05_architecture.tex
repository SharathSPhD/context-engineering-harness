...
Insertion is via a single MCP tool, \texttt{insert(...)}. Retrieval is via three MCP tools --- \texttt{retrieve\_by\_id}, \texttt{retrieve\_by\_qualifier}, and \texttt{retrieve\_under\_condition} --- none of which return \texttt{bādhita} or \texttt{evicted} rows by default. A debug flag exposes them for audit.
...
\texttt{retrieve\_under\_condition(qualificand:\ str,\ condition:\ str)} walks the \texttt{ContextStore} and returns the \emph{set} of items matching \texttt{qualificand} whose \texttt{condition} is \emph{consistent} with the supplied condition. Consistency is decided by a small rule engine:
...
\subsection{5.5 The Khyativada classifier}\label{the-khyativada-classifier}
...
Rules are as in Section 4.7. Implemented as a periodic background MCP tool \texttt{compact\_now(strategy:\ Literal{[}"adaptive",\ "lru",\ "none"{]})} that the agent or a lifecycle hook can invoke. The default policy is \texttt{adaptive}; the H7 study (Section 8.7) compares all three.
...
It is also wired into a \texttt{pretooluse-budget} hook (Section 6.4) so the host can refuse expensive tool calls when over budget.
...
\item
  \textbf{Manas} is called: it inspects the \texttt{ContextStore} snapshot via \texttt{retrieve\_by\_qualifier} and returns its attended subset.
\item
...
\item
  \textbf{Khyativada classifier} is run on Buddhi's draft answer; if \texttt{khyati\_class\ !=\ "none"} and \texttt{confidence\ \textgreater{}\ τ\_self\_check} (default 0.7), Buddhi is asked to revise once.
\item
