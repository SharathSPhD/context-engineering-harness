...
![HELMET-recall accuracy degrades sharply as the window grows from 8K to 32K tokens on the strongest current models. The Pratyakṣa-treatment line stays flat at the top; the unaided baseline line drops.](pratyaksha_images/F02_H2_helmet_by_context_length.png)

*HELMET-Recall under conflicting passages — paper Figure F02, v2.0 synthetic-fallback path. Treatment also reduces Brier score by 47 % (0.176 → 0.094) and ECE by 65 % (0.118 → 0.041) on a 1,800-example calibration slice. The live Hugging Face rerun verdict on four headline bundles sits in §4 Layer 1b. Source: §8.2, this paper.*
...
The harness was validated across three orthogonal evidence layers. Every figure and table in this section is reproduced from the [v2.1.1 preprint](https://github.com/SharathSPhD/pratyaksha-context-eng-harness/releases/tag/v2.1.1) (which supersedes the [v2.0 Zenodo record](https://zenodo.org/records/19653013)). The v2.0 synthetic-fallback omnibus (Layer 1, Layer 3, and the F12/F13 diptych below) is unchanged; the *v2.1 live-HF rerun* and the *v2.1.1 power-extension addendum* sit next to it as live companion reads on four headline bundles.
...
The Layer-1 numbers above come from the synthetic-fallback path of each adapter — generator-authored distractors, tuned so per-context-length difficulty curves track the published RULER and HELMET curves, with the real Hugging Face path sampled during development. A reasonable reviewer asks: *does the paired delta survive when you force the adapter to actually pull from Hugging Face at the current dataset commit?* So we ran a **pre-registered, strictly live** rerun of four headline bundles against live Hugging Face data — RULER at 8K and 16K tokens, TruthfulQA, and SWE-bench Verified — with an adapter-level `strict_hf=True` flag that converts any load failure into a hard `ProvenanceIntegrityError` rather than a silent synthetic fallback. The runner also probes one example per bundle before any API call goes out, reads back `provenance.source`, and aborts if it is not the literal string `"huggingface"`. Dataset commit SHAs are stamped onto every output record.

At the *N = 60*/*N = 30* paired-observation budget that a single Anthropic 5-hour rolling window affords, the live rerun produced these seven rows. The v2.1 *core4* band is the original v2.1 result; the v2.1.1 *\_ext* band is the offline-scored power extension described in the next subsection.

| bundle | band | Δ | 95 % CI | paired *p* | Cohen's *d_z* | *n_pair* | gate verdict |
| --- | --- | ---: | :---: | ---: | ---: | ---: | --- |
| RULER 8K (`H1_ruler_8192_live`) | v2.1 core4 | **+0.233** | [+0.125, +0.341] | **0.0005** | **0.547** | 60 | **combined gate met** (*d_z* ≥ 0.5 **and** *p* ≤ 0.05) |
| RULER 16K (`H1_ruler_16384_live`) | v2.1 core4 | +0.067 | [+0.003, +0.130] | 0.117 | 0.265 | 60 | directional, both gates shy |
| TruthfulQA (`H_TQA_live_v2`) | v2.1 core4 | −0.033 | [−0.126, +0.059] | 0.74 | −0.091 | 60 | null |
| SWE-bench haiku (`H_SWEB_live_n15`) † | v2.1 core4 | +0.109 | [+0.029, +0.189] | **0.032** | 0.488 | 30 | *p* met, *d_z* gate shy (0.488&nbsp;<&nbsp;0.5) — under haiku `full_errors_as_zero` |
| RULER 16K ext (`H1_ruler_16384_live_ext`) | v2.1.1 ext | +0.0485 | **[+0.010, +0.087]** | 0.064 | 0.225 | 103 | tighter interval, **CI excludes zero**, both gates still shy |
| TruthfulQA ext (`H_TQA_live_v2_ext`) | v2.1.1 ext | −0.033 | [−0.133, +0.050] | 0.74 | −0.091 | 60 | **no new rows billed** — identical to v2.1 |
| SWE-bench ext (`H_SWEB_live_ext`) | v2.1.1 ext | — | — | — | — | 1 | un-exercised — CLI-timeout fix shipped, quota halted before rebilling |

*† The SWE-bench haiku row's gate verdict is evaluated under the pre-registered `full_errors_as_zero` scoring path, which is the honest read given that ~77 % of the originally-intended haiku attempts CLI-aborted before scoring (see Appendix G for the generalisability caveat). The v2.1.1 ext row is the same prereg with the subprocess timeout raised 300 s → 900 s — no rescoring, no policy change.*

![Forest plot of all seven live-HF bundles. The four v2.1 core4 rows (RULER 8K, RULER 16K, TruthfulQA, SWE-bench-haiku) sit in the upper band; the three v2.1.1 ext rows (RULER 16K ext, TruthfulQA ext, SWE-bench ext) sit in the lower band. Marker colour encodes the preregistered combined gate (*d_z* ≥ 0.5 AND *p* ≤ 0.05): green = combined gate met, amber = directional but at least one half shy, grey = null, hollow = un-exercised. Right-edge annotations show n, paired *p*, and *d_z* for every exercised row. Solid guide at Δ = 0; dotted guide at Δ = +0.05 is a magnitude reference, not a gate.](pratyaksha_images/F14_live_hf_forest.png)

*F14 — Live Hugging Face rerun: paired Δ by bundle, 95 % CI. Bootstrap CIs ($n_\text{boot}=10{,}000$) are used for the v2.1.1 `_ext` rows; Wald CIs reconstructed from $(\Delta, d_z, n)$ are used for the v2.1 `core4` rows (which shipped without persisted bootstrap CIs in `_summary_live.json`). The gate verdict is the preregistered combined **$d_z \ge 0.5$ AND paired-permutation $p \le 0.05$**; the CI is a descriptive check, not part of the gate. Source: Table T8 of the paper and `paper/figures/F14_F15_provenance.json`.*

Reading the seven rows together: the system clears *both* halves of the preregistered combined gate on one independently-sourced live surface (**RULER 8K**, *d_z* = 0.547, *p* = 0.0005), clears the *p* half but is just shy of the *d_z* half on a second (**SWE-bench-haiku** under `full_errors_as_zero`, *d_z* = 0.488), tightens the **RULER 16K** interval to [+0.010, +0.087] (excludes zero but both gates stay shy at *n* = 103), and reports a null on **TruthfulQA** honestly (*p* = 0.74, *d_z* = −0.091). We do not claim the v2.0 omnibus on the live pull — at *N* ≈ 60–103 per bundle instead of *N* ≈ 180–700, the statistical floor is different. We do claim that the paired-delta *direction* of the synthetic-fallback path is not an artefact of the generator on the three benchmark families (RULER, SWE-bench, TruthfulQA) we could re-check inside one rolling-window's budget. The full live-HF run recipe, pre-registration, and per-bundle commit SHAs are in Appendix G of the paper (with the reproducibility commands in Appendix C).

#### v2.1.1 power-extension addendum *(post-v2.1; partial)*

After v2.1 we filed a **pre-execution power-extension amendment** to address the three contingent v2.1 reads (RULER 16K underpowered; TruthfulQA null but wide CI; SWE-bench CLI-blocked): N doubled from 15 to 30 on RULER 16K and TruthfulQA (same seeds, same models, same scorer — the v2.1 checkpoint rows seed the *\_ext* checkpoints so the first 15 cells per model × seed × condition replay as cache-hits), and the SWE-bench bundle held at N = 15 with only the CLI subprocess timeout raised from 300 s to 900 s (a pure infrastructure fix — we explicitly refused to open N on SWE-bench because the heuristic scorer is floor-ceiling-bounded and more paired observations there do not buy statistical power). Anthropic's account-level rolling quota exhausted partway through. We halted the run and re-scored the checkpoints *offline* (no further CLI traffic) with a deterministic paired-statistics pass:

- **RULER 16K extension tightens, but does not clear the gate.** Pooled n_pair = 103 (haiku fully extended to 60; sonnet seed 0 to 28; sonnet seed 1 still at 15). Paired Δ = +0.0485, *95 % bootstrap CI [+0.010, +0.087]* — **the interval now excludes zero**, but paired-permutation *p* = 0.064 and *d_z* = 0.225 still miss the preregistered combined *d_z* ≥ 0.5 AND *p* ≤ 0.05 gate. Reading: *tighter interval, same verdict — directional, still underpowered*.
- **TruthfulQA extension received no new rows.** The quota halted before any *\_ext* billing; the re-score is byte-identical to v2.1's N = 60 null (Δ = −0.033, *p* = 0.74). The null stands; a fuller power extension is queued for a future window.
- **SWE-bench infra-rescue ships but was not exercised.** The CLI-timeout fix is in the codebase and the pre-registration lock extends to the 900 s subprocess timeout, but only one paired observation billed before the quota halted. The v2.1 haiku-only row remains the operative SWE-bench live read.

![RULER 16K power-extension composition. Left panel: a forest plot with three rows — pooled (n = 103, purple), claude-haiku-4-5 (n = 60), claude-sonnet-4-6 (n = 43, partial) — each with a 95 % bootstrap CI. A dotted vertical line marks Δ = +0.05 as a magnitude reference. Right panel: a 4-row × 2-column grid of per-cell paired observation counts (haiku seed 0, haiku seed 1, sonnet seed 0, sonnet seed 1 × harness_on, harness_off). Fully extended cells (= 30) are green; partial cells are amber.](pratyaksha_images/F15_ruler16k_ext_per_model.png)

*F15 — RULER 16K power-extension composition (v2.1.1). **Left:** pooled n_pair = 103 result with 95 % bootstrap CI (purple) next to per-model splits: haiku fully extended to n_pair = 60, sonnet partial at n_pair = 43. The pooled CI *excludes* zero at [+0.010, +0.087]; the per-model CIs (haiku and sonnet, each at smaller *n*) are wider and individually cross zero at the lower edge. **Right:** per-cell paired-observation counts. Haiku's four cells each reached N = 30; sonnet seed 0 reached 30 / 28 (two SessionStart-hook aborts on the harness_off side) and sonnet seed 1 was never extended past the v2.1 baseline of 15 / 15 before the quota halted. The resulting pooled n_pair = min(60,60) + min(30,28) + min(15,15) = 60 + 28 + 15 = 103. Source: `experiments/results/p6a/_summary_live.json` (bundle `H1_ruler_16384_live_ext`).*

Net v2.1.1 effect on the paper's live-HF read: *one tighter descriptive CI on RULER 16K* (point estimate slightly lower, but bootstrap CI now excludes zero rather than straddling it; the preregistered gate is on the combined *d_z* ≥ 0.5 AND permutation *p* ≤ 0.05, not on the CI, so the gate verdict is unchanged), *the v2.1 null on TruthfulQA unchanged at N = 60 — the planned larger-N extension was halted by quota before any new rows billed*, and *a shipped-but-unexercised CLI-timeout fix on SWE-bench*. Full v2.1.1 protocol, per-model numbers, and caveats are in Appendix G of the paper. The v2.1 rows are unchanged; the three *\_ext* bundles sit alongside them as companion reads.
...
![Two charts side-by-side. Left: a bar chart of Cohen's d effect-size magnitudes by hypothesis, with H5 and H7 marked as structural. Right: a forest plot of per-hypothesis paired deltas with 95 % confidence intervals — every interval lies strictly above zero.](pratyaksha_images/F12_effect_sizes.png)

*Aggregate effect across all ten quantitative studies — paper Figure F12 (effect-size magnitudes) and Figure F13 (forest plot of per-hypothesis paired deltas with 95 % confidence intervals), both drawn from the **v2.0 synthetic-fallback** battery. The two structural-100 % rows (H5, H7) are excluded from the mean Cohen's d. The live Hugging Face companion read on four headline bundles (Layer 1b) is reported as a separate table and plot — it is not pooled into this forest. Source: §10, this paper.*
...
None of these caveats touches the central operational claim, with explicit honest exceptions stated per surface: **under a fixed token budget, the harness produces a positive directional effect on every live RULER surface tested, and clears the preregistered combined gate (*d_z* ≥ 0.5 *and* paired *p* ≤ 0.05) on RULER 8K.** RULER 16K is positive but shy of both halves of the gate at *n* = 60, and still shy of both halves at the *n* = 103 power-extension (CI excludes 0, *p* = 0.064, *d_z* = 0.225). SWE-bench-haiku passes the *p* gate but is *d_z*-shy (*d_z* = 0.488, just below 0.5) under the CLI-blocked pre-registration. The single live-HF null is TruthfulQA (paired Δ = −0.033, *p* = 0.74, *d_z* = −0.091 at *n* = 60) — reported as a null, not papered over. The hallucination surfaces clear the calibration / Brier-score axis on the synthetic-fallback bands.
