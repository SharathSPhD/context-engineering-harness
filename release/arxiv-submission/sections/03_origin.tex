...
\begin{itemize}
\item
  \textbf{\passthrough{\lstinline!attractor-flow!}} \citep{attractorflowplugin} --- a dynamical-systems-inspired orchestration loop that drove the long-running validation pipeline through stable basins (e.g.~``P6-A done'', ``P6-B done'', ``Stouffer-Z computed''). It supplied the HALT, RESUME, and basin-detection primitives for the CLI scheduler.
\item
  \textbf{\passthrough{\lstinline!ralph-loop!}} --- a self-referential iterative-development loop used during early prototyping of the harness components.
\end{itemize}

We treat these as \emph{development-time} tools, analogous to \passthrough{\lstinline!make!} or \passthrough{\lstinline!pytest!}, not as runtime dependencies of the harness. The shipped plugin contains zero references to either, and we audit this in Section 6.5 and Appendix B.
